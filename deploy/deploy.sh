...
# Preserve caller-provided AWS credentials (e.g. exported by CI over SSH)
# BEFORE sourcing .env, since .env may define these keys as empty and
# would otherwise silently overwrite/clobber the real values.
_PRESERVED_AWS_ACCESS_KEY_ID="${AWS_ACCESS_KEY_ID:-}"
_PRESERVED_AWS_SECRET_ACCESS_KEY="${AWS_SECRET_ACCESS_KEY:-}"
_PRESERVED_AWS_REGION="${AWS_REGION:-}"
_PRESERVED_AWS_ACCOUNT_ID="${AWS_ACCOUNT_ID:-}"

set -a
source "${ENV_FILE}"
set +a

# Re-apply caller-provided AWS values if .env left them blank/unset
[[ -z "${AWS_ACCESS_KEY_ID:-}" ]] && AWS_ACCESS_KEY_ID="${_PRESERVED_AWS_ACCESS_KEY_ID}"
[[ -z "${AWS_SECRET_ACCESS_KEY:-}" ]] && AWS_SECRET_ACCESS_KEY="${_PRESERVED_AWS_SECRET_ACCESS_KEY}"
[[ -z "${AWS_REGION:-}" ]] && AWS_REGION="${_PRESERVED_AWS_REGION}"
[[ -z "${AWS_ACCOUNT_ID:-}" ]] && AWS_ACCOUNT_ID="${_PRESERVED_AWS_ACCOUNT_ID}"
export AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_REGION AWS_ACCOUNT_ID
...
if [[ -n "${AWS_ACCESS_KEY_ID:-}" && -n "${AWS_SECRET_ACCESS_KEY:-}" && -n "${AWS_REGION:-}" && -n "${AWS_ACCOUNT_ID:-}" ]]; then
  ...
  info "Skipping ECR login: one or more of AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY / AWS_REGION / AWS_ACCOUNT_ID is empty."
  info "If the image lives in a private ECR repo, the upcoming pull will fail with a 'no basic auth credentials' error."
fi
